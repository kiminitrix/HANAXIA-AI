         ...
         <div className="max-w-6xl mx-auto w-full px-4 pt-8 pb-4 flex-1 flex flex-col justify-center">
            ...
            {!videoUri && !isLoading && !error && (
               <div className="flex flex-col items-center justify-center animate-fade-in text-center">
                  ...
                  <h2 className="text-2xl font-medium text-gray-200">Cinematic AI Video.</h2>
                  <p className="text-gray-500 mt-2">Create amazing videos with Veo.</p>
                  <div 
                    className="mt-4 text-xs text-amber-500/80 bg-amber-900/10 px-3 py-1 rounded-full cursor-pointer hover:bg-amber-900/20 transition-colors"
            ...
            {/* Error State */}
            {error && (
              <div className="p-4 bg-red-900/20 border border-red-900/50 text-red-200 rounded-xl text-center max-w-md mx-auto">
            ...
            {isLoading && (
                <div className="flex flex-col items-center justify-center">
                    <div className="flex gap-8 justify-center w-full">
                        <div className="w-[400px] h-[225px] rounded-2xl bg-white/5 animate-pulse flex items-center justify-center border border-white/10">
                            <div className="text-purple-500 text-4xl animate-spin">{ICONS.loading}</div>
                        </div>
                    </div>
                    <p className="mt-6 text-gray-400 font-medium">Generating video... (this may take a minute)</p>
                </div>
            )}

            {/* Results Area */}
            {!isLoading && videoUri && (
               <div className="w-full max-w-4xl mx-auto rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black">
                 <video 
                 ...
                 />
                 <div className="p-4 bg-[#0a0a0a] flex justify-between items-center border-t border-white/10">
                    <div className="text-sm font-medium text-gray-300 truncate max-w-lg">{prompt}</div>
                    <a 
                      href={`${videoUri}&key=${process.env.API_KEY}`} 
                      download 
                      className="px-4 py-2 bg-white text-black font-semibold rounded-lg hover:bg-gray-200 transition-colors text-sm flex items-center gap-2"
                    >
             ...
             <div className="bg-[#1a1a1a] rounded-t-2xl border-x border-t border-white/10 px-4 py-3 flex items-center justify-between animate-slide-up">
                <div className="flex items-center gap-3">
                   ...
                   <div className="text-xs text-gray-400">
                      <div className="font-semibold text-white">Start Image</div>
